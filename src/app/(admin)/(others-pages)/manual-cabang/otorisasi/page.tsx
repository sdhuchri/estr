...
import { getManualCabangOtorisasi } from "@/services/manualCabang";
import ManualCabangOtorisasiClient from "./ManualCabangOtorisasiClient";
...
async function ManualCabangOtorisasiData() {
  ...
  try {
    if (branchCode) {
      ...
      const response = await getManualCabangOtorisasi(branchCode);
  ...
  return (
    <FadeInWrapper>
      <ManualCabangOtorisasiClient initialData={data} />
    </FadeInWrapper>
  );
}

// Main page component with PPR
export default function ManualCabangOtorisasiPage() {
  return (
    <Suspense fallback={
      <DataTableSkeleton
        rows={8}
...
export const metadata = {
  title: "STR | Manual Cabang Otorisasi",
  description: "Kelola data laporan manual cabang"
};
