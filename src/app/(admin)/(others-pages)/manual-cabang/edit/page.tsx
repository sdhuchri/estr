...
import { getManualCabangEdit, updateManualCabang } from "@/services/manualCabang";
...
export default function ManualCabangEditPage() {
  const router = useRouter();
  const toast = React.useRef<Toast>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [data, setData] = useState<ManualCabangData[]>([]);
  ...
  const [editFormData, setEditFormData] = useState({
    ...
    jamHubungiNasabah: new Date().toLocaleTimeString("id-ID", { hour12: false }),
  ...
  useEffect(() => {
    document.title = "ESTR | Manual Cabang Edit";
  ...
  const handleRowAction = (action: string, item: ManualCabangData) => {
    if (action === "edit") {
      ...
      setEditFormData({
        ...
        jamHubungiNasabah: item.JAM_HUB_NASABAH || new Date().toLocaleTimeString("id-ID", { hour12: false }),
  ...
  const performSave = async () => {
    ...
    try {
      ...
      const updateData = {
        penjelasan: editFormData.penjelasanCSO,
        tgl_hub: formattedDate,
        jam: editFormData.jamHubungiNasabah,
        trans_mencurigakan: editFormData.transaksiMencurigakan,
        id_laporan: selectedData.ID_LAPORAN,
        indikator: selectedData.INDIKATOR,
        no_cif: selectedData.NO_CIF
      };

      const response = await updateManualCabang(updateData);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <FormField label="Tgl Menghubungi Nasabah">
                <DatePickerInput
                  ...
                  placeholderText={new Date().toLocaleDateString("id-ID")}
              ...
              <FormField label="Jam Menghubungi Nasabah">
                <TimeInput24
                  ...
                  placeholder={new Date().toLocaleTimeString("id-ID", { hour12: false })}
                />
              </FormField>
            </div>
