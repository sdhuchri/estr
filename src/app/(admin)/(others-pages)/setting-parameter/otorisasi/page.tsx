...
import { getParameterRedflagOtorisasi } from "@/services/parameterRedflag";
import SettingParameterOtorisasiClient from "./SettingParameterOtorisasiClient";
...
async function SettingParameterOtorisasiData() {
  ...
  try {
    // Fetch data for pending authorization
    const response = await getParameterRedflagOtorisasi();
    if (response.status === "success" && response.data && response.data.length > 0) {
      ...
      parameters = {
        ...
        filterCabang: apiData.TARIK_SETOR_NOMINAL_FILTER_CBG || "",
        ...
        kategoriBlacklist: apiData.DB_SUSPECT_KATEGORI_BLACKLIST || "",
  ...
  return <SettingParameterOtorisasiClient initialData={parameters} userId={userId} />;
}

// Main page component
export default function SettingParameterOtorisasiPage() {
  return (
    <SettingParameterOtorisasiData />
  );
}

// Add metadata
export const metadata = {
  title: "STR | Otorisasi Setting Parameter",
  description: "Otorisasi perubahan parameter sistem"
};
